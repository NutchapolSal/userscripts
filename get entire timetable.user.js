...
(async function() {
    ...
    for (const v of document.querySelectorAll("div.v-dialog")) {
        ...
        const courseName = v.querySelector("h3.mt-2.deep-orange--text").innerText
        const courseSection = v.querySelector("h3.mt-2.mb-2").innerText
        for (const v2 of v.querySelectorAll("div.v-card__text > div.text-center")) {
            ...
            events.push(`${courseName}\t${courseSection}\t${eventDate.getDate()}-${eventDate.getMonth() + 1}-${eventDate.getFullYear()}\t${eventDate.getHours()}:${eventDate.getMinutes()}\t${eventTimeEnd}`)
        }
    }
